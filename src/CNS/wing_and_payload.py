...
def control_surface_ratio_conventional(data, PL = 1):
...
def stability_surface_ratio_conventional(data, PL = 1):
...
def control_surface_ratio_canard(data, PL = 1):
    ...
    canard = lambda xw, xpl: data['Aero']['CL_c']/data['Aero']['CL_A-h']*data['C&S']['Sc']/data['FPP']['S [m^2]']*((data['C&S']['Canard'][0]-1)/chord - xw - data['Aero']['x_ac'])

    return lambda xw, xpl: coefficient(xw, xpl) * (cg(xw, xpl) - xw - data['Aero']['x_ac'] + data['Aero']['Cm_ac'] / data['Aero']['CL_A-h'] - thrust - canard(xw, xpl)), cg


def stability_surface_ratio_canard(data, PL = 1):
    ...
    return lambda xw, xpl: coefficient*(((cg(xw, xpl) + SM)*(1+data['Aero']['CL_alpha_c']/data['Aero']['CL_alpha_A-h']*data['C&S']['Sc']/data['FPP']['S [m^2]']) - (xw + data['Aero']['x_ac'] + (data['C&S']['Canard'][0]-1)/chord*data['Aero']['CL_alpha_c']/data['Aero']['CL_alpha_A-h']*data['C&S']['Sc']/data['FPP']['S [m^2]']))/
                                        (cg(xw, xpl) + SM - (data['C&S']['H Wing'][0]-1)/chord))


def plot_conventional(data = GoogleSheetsDataImport(SPREADSHEET_ID, *SHEET_NAMES).get_data()):
    ...
    fc_Sh_full, cgfull = control_surface_ratio_conventional(data, PL=1)
    fc_Sh_empty, cgempty = control_surface_ratio_conventional(data, PL=0)
    ...
    ax.set_zlabel('Sh/S')
    ax.title.set_text('Controllability - Conventional')
    # ax.set_ylim(0, 1)
    ...
    ax.set_ylabel('Payload Position')
    ax.set_zlabel('X CG')
    ax.title.set_text('C.o.G Position - Conventional')
    ...
    fs_Sh_full = stability_surface_ratio_conventional(data, PL=1)
    fs_Sh_empty = stability_surface_ratio_conventional(data, PL=0)
    ...
    ax.set_ylabel('Payload Position')
    ax.set_zlabel('Sh/S')
    ax.title.set_text('Stability - Conventional')
...
def plot_canard(data=GoogleSheetsDataImport(SPREADSHEET_ID, *SHEET_NAMES).get_data()):
    xwing = np.linspace(0, 3., 50)
    xpayload = np.linspace(0, 3., 50)
    ...
    fc_Sh_full, cgfull = control_surface_ratio_canard(data, PL=1)
    fc_Sh_empty, cgempty = control_surface_ratio_canard(data, PL=0)
    ...
    ax.set_ylabel('Payload Position')
    ax.set_zlabel('Sh/S')
    ax.title.set_text('Controllability - Canard')
    # ax.set_ylim(0, 1)
    ...
    ax.set_ylabel('Payload Position')
    ax.set_zlabel('X CG')
    ax.title.set_text('C.o.G Position - Canard')
    ...
    fs_Sh_full = stability_surface_ratio_canard(data, PL=1)
    fs_Sh_empty = stability_surface_ratio_canard(data, PL=0)
    ...
    ax.set_ylabel('Payload Position')
    ax.set_zlabel('Sh/S')
    ax.title.set_text('Stability - Canard')
